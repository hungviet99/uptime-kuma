ARG BASE_IMAGE=hungnv/uptime-kuma:debian-base
...
############################################
FROM hungnv/uptime-kuma:builder-go AS build_healthcheck

############################################
# Build in Node.js
############################################
FROM hungnv/uptime-kuma:debian-base AS build
USER node
WORKDIR /app
...
# RUN npm ci --omit=dev
# # RUN apt --yes --no-install-recommends install git
# RUN npm run setup
COPY . .
# COPY .git ./
RUN npm ci --production && npm run download-dist
# RUN npm run setup
# COPY . .
...
############################################
FROM hungnv/uptime-kuma:debian-base AS pr-test2
WORKDIR /app
ENV PUPPETEER_SKIP_CHROMIUM_DOWNLOAD=1
...
############################################
# FROM hungnv/uptime-kuma:debian-base AS upload-artifact
# WORKDIR /
# RUN apt update && \
...
# RUN /app/extra/upload-github-release-asset.sh github_api_token=$GITHUB_TOKEN owner=hungviet99 repo=uptime-kuma tag=$VERSION filename=/app/$DIST